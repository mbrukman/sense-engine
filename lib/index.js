...
Engine.prototype.iframe = function(params) {
  if (params.hasOwnPropertyName('src') && params.hasOwnPropertyName('srcdoc')) {
    throw new Error("Either 'src' or 'srcdoc' should be provided, but not both.")
  }
  this._output('iframe', {
    src: params.src, 
...
Engine.prototype.image = function(params) {
  if (/^http:/.exec(params.src)) {
    throw new Error("URL sources should be https, not plain http.")
  }
  this._output('image', {
    src: params.src,
    height: params.height,
