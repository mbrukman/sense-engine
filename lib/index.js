var EventEmitter = require('events').EventEmitter;
var _ = require('underscore');
var fs = require('fs');

var Dashboard = function Dashboard(implementation, handler, startupScript, startupCode, batch) {
  this._implementation = implementation;
  this._commandQueue = [];
  this._executing = false;
  this._cell = 0;
  this._lastInput = 1;
  this._lastType = null;
  this._textBuffer = '';
  this._started = false;
  this._executing = false;
  this._batch = batch;
  this._throttledText = _.debounce(this._flushText.bind(this), 300);
  if (startupScript !== undefined && startupScript !== 'undefined') {
    this._startupScript = fs.readFileSync(startupScript).toString();
  }
  else if (startupCode !== undefined && startupCode !== 'undefined') {
    this._startupScript = startupCode;
  }

  // Output from worker dashboards gets tagged with an ID, but not output
  // from master dashboards.
  if (process.env.SENSE_MASTER_ID) {
    this._id = process.env.SENSE_DASHBOARD_ID;
  }
  else {
    this._id = null;
  }

  // The following functions should be redefined when 
  // implementation(this) is called.
  this.chunk = function(input, cb) {
    cb([input]);
  };
  this.complete = function(input, cb) {
    cb([]);
  };
  this.execute = function(input, next) {
    undefined;
  };
  this.interrupt = function() {
    undefined;
  };

  handler(this);  
  implementation(this);
};

for (var prop in EventEmitter.prototype) {
  Dashboard.prototype[prop] = EventEmitter.prototype[prop];
}

var eraserToken = new String('erase');

Dashboard.prototype._output = function(params) {
  this._lastType = params.type;
  this.emit('output', {
    mime: params.mime,
    data: params.data,
    cell: this._cell,
    input: params.input || false,
    id: this._id 
  });
}

Dashboard.prototype._flushText = function() {
  if (this._textBuffer.length) {
    if (this._lastType !== 'text') this._increment();
    this._output({type: "text/plain", data: this._textBuffer});
    this._textBuffer = '';
  }
};

Dashboard.prototype._increment = function() {
  this._cell += 1;
};

Dashboard.prototype.output = function(params) {
  if (params.type === "text/plain") {
    this._textBuffer += text;
    this._throttledText();
  }
  else {
    this._flushText();
    this._increment();
    this._output(params);
    if (params.type === "application/error") {
      this._commandQueue = [];
      this._errored = true;
    }    
  }
};

Dashboard.prototype.input = function(input, overwriteLast) {
  this.chunk(input, (function(chunks) {
    if (overwriteLast) {
      this._commandQueue.push([eraserToken, this._lastInput]);
    }
    this._lastInput = this._cell+1;
    this._commandQueue = this._commandQueue.concat(chunks);
    if (!this._executing) {
      this._next();
    }
  }).bind(this));
};

Dashboard.prototype._erase = function(cell) {
  this.emit('erase', cell);
};

Dashboard.prototype._next = function() {
  this._flushText();
  if (this._commandQueue.length) {
    var cmd = this._commandQueue.shift();
    if (cmd[0] === eraserToken) {
      for (var cell = cmd[1]; cell <= this._cell; cell++) {
        this._erase(cell);
      }
      this._cell = cmd[1]-1;
      this._next();
    }
    else {
      this.emit('executing');
      this._executing = true;
      this.execute(cmd, this._next.bind(this));
    }
  } else {
    this.emit('ready');
    this._executing = false;
  }
};

Dashboard.prototype._interrupt = function() {
  this._commandQueue = [];
  this.warning('Interrupting dashboard.');
  this.interrupt();
};

Dashboard.prototype.ready = function() {
  if (!this._started) {
    this._started = true;
    this.emit('started');
    this.emit('ready');
    if (this._startupScript) {
      this.chunk(this._startupScript, (function(chunks) {
        this._commandQueue = chunks;
        this._next();
      }).bind(this));
    }
  }
  if (this.batch) {
    process.exit(this.errored);
  }
};

var rep = function rep(str, n) {
  var out = "";
  for (var i = 0; i < n; i++) {
    out += str;
  }
  return out;
};

var testHandler = function(whenReady) {
  return function(dashboard) {
    // Output events: 'ready', 'erase', 'executing', 'output'
    // Input methods: 'complete', 'input', 'interrupt'
    var tester = function(input, done) {
      var out = [];
      dashboard.removeAllListeners('output');
      dashboard.removeAllListeners('ready');
      dashboard.on('output', function(data) {
        out.push(data);
      });
      dashboard.on('ready', function() {
        done(out);
      });
      dashboard.input(input);
    };
    dashboard.on('ready', (function() {whenReady(tester)}));
  };
};

var replHandler = function replHandler(options) {
  options = options || {};
  return function(dashboard) {
    var path = require('path');
    var readline = require('readline');
    var defaultPrompt = "> ";
    var prompt = defaultPrompt;
    var lastDisplayedPrompt = prompt;
    var multiline = false;
    var multilineBuffer = [];

    var rl = readline.createInterface({
      input: process.stdin,
      output: process.stdout,
      completer: function(substr, cb) {
        dashboard.complete(substr, function(completion) {
          cb(null, [completion, substr]);
        });
      }
    });

    process.stdin.on('keypress', function(char, key) {
      if (key && key.ctrl && !key.meta && key.name === 'v' && !key.shift) {
        if (!multiline) {
          multilineBuffer = [];
          prompt = rep(".", lastDisplayedPrompt.length-1) + " ";
        }
        multiline = true;
      }
      else if (!multiline) prompt = defaultPrompt;
    });

    var lineProcessor = function (answer) {
      if (multiline) {
        if (answer.trim().length === 0) {
          multiline = false;
          prompt = defaultPrompt;
          dashboard.input(multilineBuffer.join('\n'));
        }
        else {
          multilineBuffer.push(answer);
          lastDisplayedPrompt = prompt;
          rl.question(prompt, lineProcessor);
        }
      }
      else {
        dashboard.input(answer);
      }
    };
    dashboard.on('ready', function() {
      if (!options.pretty) console.log('status: ready');
      lastDisplayedPrompt = prompt;
      rl.question(prompt, lineProcessor);
    });

    dashboard.on('output', function (data) {
      if (data.mime === 'text/prompt' && !multiline) {
        prompt = data.data;
      }
      else {
        if (options.pretty) {
          if ({
            "text/plain": true,
            "text/comment": true,
            "text/markdown": true,
            "text/latex": true
          }[data.mime]) {
            console.log(data.data);
          }
          else if (data.mime.indexOf("text/")==0) {
            undefined;
          }
          else if ({
            "application/error": true,
            "application/warning": true
          }[data.mime]) {
            console.log(data.data.message);
          }
          else {
            console.log("[" + data.mime + "]");
          }
        }
        else {
          console.log(data);
        }
      }
    });

    dashboard.on('erase', function(cell) {
      if (!options.pretty) console.log('Erasing cell', cell);
    });

    dashboard.on('executing', function () {
      if (!options.pretty) console.log('status: executing');
    });

    rl.on('SIGINT', function() {
      dashboard.interrupt()
    });
  }
};

var repl = function repl(implementation, startupScript, options) {
  new Dashboard(implementation, replHandler(options), startupScript);
};

var test = function test(implementation, whenReady) {
  new Dashboard(implementation, testHandler(whenReady));
};

exports.Dashboard = Dashboard;
exports.repl = repl;
exports.test = test;