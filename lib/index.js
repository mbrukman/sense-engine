...
Engine.prototype._complete = function(input, cursorPos) {
  this.complete(input, cursorPos, function(token, completion) {
    this.emit('completions', {input: input, token: token, completion: completion})
  });
};
...
Engine.prototype.complete = function(input, cursorPos, cb) {
  cb({token: input.slice(0, cursorPos), completion: []});
  return this;
};
...
Engine.prototype.repl = function() {
  ...
  var rl = readline.createInterface({
    ...
    completer: function(substr, cb) {
      thisEngine.complete(substr, substr.length, function(info) {
        console.log("\nCompletion prefix is " + substr.slice(info.token.start, info.token.end) + "\n");
        cb(null, [info.completions, substr]);
      });
    }
  });
