...
Engine.prototype._output = function(data) {
  this._flushText();
  this._increment();
  this.emit('output', {
    type: 'html',
...
// Text, html, and error are the primary types.
...
Engine.prototype.error = function(message, details) {
  var html = '<sense-error><span>' + _.escape(message) + '</span><span>' + _.escape(details) + '</span></sense-error>';
  this._output(html);
  this._commandQueue = [];
  this._errored = true;
  return this;
...
Engine.prototype.code = function(code, language) {
  var html = '<sense-code language="' + _.escape(language) + '">' + _.escape(code) + '</sense-code>';
  this._output(html);
  return this;
};

Engine.prototype.comment = function(comment) {
  var html = '<sense-comment>' + _.escape(comment) + '</sense-comment>';
  this._output(html);
  return this;
};

Engine.prototype.help = function(help) {
  var html = '<sense-help>' + _.escape(help) + '</sense-help>';
  this._output(html);
  return this;
};

Engine.prototype.markdown = function(markdown) {
  var html = '<sense-markdown>' + _.escape(markdown) + '</sense-markdown>';
  this._output(html);
  return this;
};

Engine.prototype.warning = function(message, details) {
  var html = '<sense-warning><span>' + _.escape(message) + '</span><span>' + _.escape(details) + '</span></sense-warning>';
  this._output(html);
  return this;
};

Engine.prototype.prompt = function(prompt) {
  var html = '<sense-prompt>' + _.escape(prompt) + '</sense-prompt>';
  this._output(html);
  return this;
};
